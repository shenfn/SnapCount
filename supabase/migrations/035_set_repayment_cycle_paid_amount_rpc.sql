...
begin
  ...
  v_remaining := greatest(round(coalesce(v_cycle.statement_amount, 0)::numeric - v_paid, 2), 0);

  if p_status is not null and p_status not in ('partial_paid', 'carried_over', 'paid', 'ignored') then
  ...
  update public.account_entries
     set is_voided = true,
         voided_reason = 'repayment_cycle_reconfirmed'
   where user_id = auth.uid()
     and source_table = 'account_repayment_cycles'
     and source_id = v_cycle.id
     and is_voided = false;
