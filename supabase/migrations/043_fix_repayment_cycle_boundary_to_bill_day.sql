...
with system_cycle_amounts as (
  ...
  where cycle.source = 'system'
    and cycle.status in ('pending', 'due_today', 'overdue_unconfirmed')
    and cycle.paid_amount = 0
  group by cycle.id, account.bill_day, account.payment_due_day
)
update public.account_repayment_cycles cycle
set statement_start_date = system_cycle_amounts.statement_start_date,
    statement_end_date = system_cycle_amounts.statement_end_date,
    due_date = system_cycle_amounts.due_date,
    statement_amount = system_cycle_amounts.amount,
