import { normalizeEntityText } from './index.mjs'

const LEGAL_NAME_SUFFIXES = [
  '股份有限责任公司', '有限责任公司', '股份有限公司', '集团有限公司', '个人独资企业',
  '普通合伙企业', '有限合伙企业', '个体工商户', '合伙企业', '有限公司',
...
function historicalVariantBasis(currentMerchant, priorMerchant) {
  ...
  if (current.core === prior.core && (current.removed || prior.removed)) return 'legal_suffix'
...
export function resolveMerchant(value, aliasMap) {
  ...
  if (matched) return { raw_name, normalized_key, ...matched, resolution: 'configured_alias', confidence: 1 }
  return {
    raw_name,
    normalized_key,
...
export function summarizeMerchantObservation(currentMerchant, priorMerchants = []) {
  ...
  let ambiguous = false
  let matchingPrior = entityId ? priorMerchants.filter(merchant => merchant?.entity_id === entityId) : []
  ...
  const observationSources = ambiguous ? [...variantMatches.map(match => match.merchant), currentMerchant] : [...matchingPrior, currentMerchant]
