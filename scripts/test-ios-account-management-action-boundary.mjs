...
test('A4-IOS-004 AppState delegates account writes and invalidates on reset', async () => {
  ...
  const nextBlock = source.indexOf('func loadUnboundRecords', archiveStart + 1)
  ...
  assert.ok(source.includes('AccountManagementActionUseCase'), 'AppState use case adapter is missing')
