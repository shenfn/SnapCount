...
- PWA `npm run build` 通过；仅有既有 vConsole `eval` 和大 chunk 警告。
- iOS Simulator Build 和 48 项单元测试通过，GitHub run `29556537369`。
- 数据库迁移 `068_atomic_staging_archive_and_tenant_hashes.sql` 已先在远程事务中执行并回滚验证，再正式应用并登记为 `068/068`。
- 远程只读检查确认 `archive_staging_record`、`transactions.staging_record_id`、`income_records.staging_record_id` 均已生效。
- Edge Function 部署成功，GitHub run `29556988394`。
- 本批未触发 TestFlight，继续遵守低频上传规则。

第三批数据完整性修复（2026-07-17）：

- 对照 PWA `useStore.loadData` 的月份边界，将当前月消费、收入和通用记录改为按月份服务端过滤，并使用 500 条分页持续读取，移除 iOS 原有 `80/40/40` 截断。
- 记录页历史月份改为真实远端请求；当前月继续复用 Dashboard，历史月份按月缓存，加载失败可单独重试。
- Dashboard 明确标记消费、收入、通用记录和中转站的失败分区；部分失败时只保留失败分区的旧数据，成功分区继续更新，避免残缺快照覆盖完整缓存。
- 账户流水、还款账期和支付记录继续并行独立加载，但错误不再被 `try?` 吞成空数组；详情页逐区显示失败原因和重试入口。
- 提交：`8a623cb`、`c7eb346`、`88652a3`。
- iOS Simulator Build 和 50 项单元测试通过，GitHub run `29557768730`；本批未触发 TestFlight。

第四批记录与数据域详情对齐（2026-07-17）：

- 阅读详情优先读取 PWA 当前 `reading_minutes/pages`，兼容历史 `duration_minutes/pages_read`；运动热量优先读取 `calories`，兼容 `calories_kcal`；睡眠兼容分钟和小时字段。
- 钱包记录详情使用账户、金额、记录类型、还款日和关联账户等业务字段，不再直接暴露原始 payload key；未知自定义域根据 `schema_json/display_json` 显示主事实和主维度。
- 当前月 Dashboard 和历史月份查询都随列表生成完整结构化详情缓存；点击记录先立即显示字段，图片只对可见记录预热，其余图片进入详情后渐进补全。
- 数据域详情按 PWA schema 计算本月主事实总量、最高、平均、周趋势和主维度分布；最近记录同步展示主事实值。
- 提交：`8f54ca8`、`2bacd60`、`802cd10`。
- iOS Simulator Build 和单元测试通过，GitHub run `29559162502`；本批未触发 TestFlight。

第五批月度报告与跨域图表（2026-07-17）：

- 分析页增加“月度报告 / 联动分析”视图切换，月报成为默认入口，原有 7/14/30 天联动和 AI 解读继续保留。
- 月报复用月份分页与结构化详情缓存，支持历史月份、数据域切换、主指标、周趋势、分类排行、消费渠道、支付方式和最近记录。
- 补齐 PWA 已有的“睡眠时长 × 饮食热量”和“饮食热量趋势”图；睡眠与饮食使用各自峰值归一化，避免小时和千卡共用绝对坐标造成误读。
- 提交：`a84f5cc`、`d2b9b67`。
- iOS Simulator Build 和单元测试通过，GitHub run `29560386877`；本批未触发 TestFlight。

阶段性真机版本：TestFlight 工作流 `29579505811` 于 2026-07-17 成功完成 Archive、IPA 导出和 Apple 上传，HEAD 为 `bd20422`。本次上传后暂停重复 TestFlight，优先收集真机功能、导航、数据和布局问题。
