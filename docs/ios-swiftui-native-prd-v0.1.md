...
9. **Build 按 commit 验证，TestFlight 按阶段验收。** 每个 commit 必须经过 GitHub iOS Build；多个小 commit 完成一个完整阶段后，再统一进行一次 TestFlight 和用户截图复核。禁止逐 commit 上传 TestFlight。
...
每个子批次继续执行 GitHub iOS Build；同一阶段的子批次完成后统一执行一次 TestFlight 和真机复核。任何子批次都不得同时重写稳定 multipart 识别链路、财务 RPC 和页面视觉，以免无法定位回归来源。
