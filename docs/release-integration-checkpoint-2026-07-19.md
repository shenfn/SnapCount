...
- 不合并或推送到 `main`。
- 不触发 TestFlight。
- 不把 `supabase/deferred-migrations/080_qwen_only_constraints.sql` 移入自动迁移目录。
...
1. `078_registration_consent_and_private_defaults.sql`：兼容型迁移。增加同意记录字段，默认关闭 AI 日志，关闭现有账号日志，并收紧 `handle_new_user` ACL。
2. 部署新 PWA 和两个 Edge Functions，完成生产冒烟测试。
3. `079_qwen_defaults_and_normalization.sql`：只归一数据和默认值，不增加会拒绝旧客户端写入的约束。
4. `080_qwen_only_constraints.sql`：保持延后。旧客户端全部退役并再次复审前不得执行。
...
- 迁移版本唯一性检查
- iOS Build 和单元测试
- `supabase db push --dry-run` 只显示预期的 078/079，且不包含 080 或重复历史迁移
