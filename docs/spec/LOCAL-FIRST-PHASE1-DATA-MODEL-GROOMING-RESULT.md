...
`Profile` 和 `Account` 是本地身份与消费账务的支撑实体，不是第六、第七个内容域。收入（income）和钱包快照（wallet）已有云端业务语义，但不进入本轮五域本地正式事实承诺。
...
当前 iOS 消费实现要求创建时提供账户，且 `local_expenses.account_id` 为非空；既有本地生命周期规格允许消费暂不绑定账户。后续实现前应按本 Grooming 目标统一为：消费事实可先保存，`account_id` 可空；只有已绑定账户的消费才创建有效账户流水。若产品继续采用“本地消费必须先选账户”，应把它记录为 UI 门禁，而不是把账户关系误当成消费事实的必要字段。
...
目标模型中的 `source_kind`、`domain_version`、候选到正式记录的关系字段目前未完整存在于通用 iOS 本地模型，属于后续实现缺口，不在本轮直接补代码。
...
- 置信度阈值属于域配置，不应由所有域共享一个无法解释的硬编码值。当前 Supabase 语义为：food 0.80，sport/sleep/reading 0.75；iOS 通用实现固定 0.80，属于待收敛差距。建议本地采用“域阈值优先、无配置时 0.80 兜底”。
...
| DM-GAP-02 | 通用本地记录缺少统一 source、domain version、staging target 关系 | AI 来源和确认幂等难以审计 | 在本地模型 Slice 中补齐，不改同步协议 |
| DM-GAP-03 | Supabase 域阈值与 iOS 固定 0.80 不一致 | 高/低置信路由跨端不一致 | 以域配置为事实来源，补跨端场景测试 |
...
| DM-GAP-10 | 当前消费本地实现要求账户非空，但既有规格允许未绑定消费 | 记录事实与账务关系耦合 | 进入 Expense 模型修正切片，先不改本轮代码 |
