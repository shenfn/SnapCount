...
test('PWA renders owner planner feedback before acknowledging its exposure', async () => {
  ...
  assert.match(detail, /shouldAcknowledgePlannerFeedback\(\{[\s\S]*?delivery:\s*plan/)
  assert.match(detail, /:reviewable="aiFeedbackReviewable"/)
  assert.match(detail, /isPlannerDeliveryReviewable\(recordExpressionPlan\.value\)/)
  assert.match(detail, /!aiFeedbackReviewable\.value/)
  assert.match(detail, /onCleanup\(\(\)\s*=>\s*\{/)
  assert.match(detail, /controller\.abort\(\)/)
  assert.match(detail, /waitForVisibleElement\(plannerPresentationElement\(\)/)
...
test('PWA keeps companion-target review controls inside companion while card targets retain their own slot', async () => {
  ...
  const pendingPlannerIndex = pending.indexOf('v-if="aiFeedback"', pendingCompanionReviewIndex)
...
test('PWA pending records expose the same visible acknowledgement and review loop', async () => {
  ...
  assert.match(pending, /store\.loadRecordExpressionPlan\(recordId,\s*\{[\s\S]*?recordKind,[\s\S]*?signal,[\s\S]*?force:\s*forceThisAttempt/)
  assert.match(pending, /waitForVisibleElement\(pendingPlannerPresentationElement\(\)/)
  assert.match(pending, /if \(plannerTargetsCompanion\.value\) return pendingCompanionRef\.value/)
  assert.match(pending, /expressionRecordKind\.value,\s*companionMessage\.value/)
  assert.match(pending, /force:\s*companionChangedInPlace/)
